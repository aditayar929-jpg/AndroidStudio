...
constructor(
  ...
  /**
   * A callback interface to notify the host (usually an Activity) about file modification status
   * changes.
   */
  ...
  /**
   * The [CoroutineScope] for the editor. All the jobs in this scope are cancelled when the editor
   * is released.
   */
  ...
  /**
   * Setup the editor language for the given [file]. This applies a proper [Language] and the color
   * scheme to the editor.
   */
  ...
  private fun getHistoryIndex(undoManager: UndoManager): Int {
    if (mUndoManagerField == null) {
      try {
        mUndoManagerField =
            undoManager.javaClass.getDeclaredField("stackPointer").apply { isAccessible = true }
  ...
  protected open suspend fun onFindDefinitionResult(
      ...
      withContext(Dispatchers.Main) {
        ...
        if (result == null) {
          // Log warning but maybe not flash error to user to avoid spam if it just didn't find
          // anything
